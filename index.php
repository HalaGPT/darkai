...
setLoading(btn, true);
let apiUrl = uploadedUrlImg2Vid ? `https://sii3.moayman.top/api/img-to-video.php?text=${encodeURIComponent(text)}&link=${encodeURIComponent(uploadedUrlImg2Vid)}` : `https://sii3.moayman.top/api/text-to-video.php?text=${encodeURIComponent(text)}`;
try{
...
setLoading(btn, true);
const apiUrl = `https://sii3.moayman.top/api/text-to-video.php?text=${encodeURIComponent(text)}`;
try{
const r = await fetch(apiUrl);
const data = await r.json();
let videoUrl = data.url || data.video || (data.data && (data.data.url||data.data.video));
